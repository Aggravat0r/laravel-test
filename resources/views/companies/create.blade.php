...
@section('title', __("messages.company_create"))

@section('content_header')
    <h1 class="m-0 text-dark">{{__("messages.company_create") }}</h1>
@stop
                ...
                <form method="POST" action="{{ url("/admin/companies") }}">
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" maxlength="255" value=" {{ old('name') }}">
                        </div>
                        <div class="form-group">
                            <label for="city_name">{{ __("messages.city_name") }}</label>
                            <input type="text" class="form-control @error('city_name') is-invalid @enderror" id="city_name" name="city_name" maxlength="255" value=" {{ old('city_name') }}">
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="website" class="form-control @error('website') is-invalid @enderror" id="exampleInputEmail1" value=" {{ old('website') }}">
                        </div>
                        <div class="form-group">
                            <label for="exampleInputEmail1">{{ __("messages.category") }}</label>
                            <select name="category" class="form-control @error('category') is-invalid @enderror">
                                @foreach($categories as $category)
                                    <option @if($category == old('category')) selected @endif>{{ $category }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group mb-0">
                            ...
                            <select name="subcategory" class="form-control @error('subcategory') is-invalid @enderror">
                                @foreach($subcategories as $subcategory)
                                    <option @if($subcategory == old('subcategory')) selected @endif>{{ $subcategory }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
